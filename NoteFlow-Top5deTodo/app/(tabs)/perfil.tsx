...
export default function Estadisticas() {
  ...
  async function cambiarFoto() {
    ...
    try {
      ...
      await updateDoc(doc(db, 'usuarios', usuario.uid), { avatarUrl: publicUrl });
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
            ...
            return (
              <View key={cat.nombre} style={styles.barraFila}>
                ...
                <View style={styles.barraTrack}>
                  <View style={[styles.barraRelleno, { width: `${(cat.cantidad / maxCantidad) * 100}%`, backgroundColor: color }]} />
                </View>
      ...
      <Button mode="outlined" onPress={confirmarCerrarSesion} icon="logout" textColor="#ff4444" style={styles.botonCerrarSesion}>
        Cerrar sesión
      </Button>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  perfilSeccion: { alignItems: 'center', gap: 8, marginBottom: 8 },
  avatarIcon: { backgroundColor: '#16162a' },
  cambiarFotoTexto: { color: '#534AB7', fontSize: 13, marginTop: 4 },
  perfilNombre: { color: '#fff', fontSize: 20, fontWeight: 'bold' },
  perfilEmail: { color: '#9090b0', fontSize: 14 },
  titulo: { color: '#fff', fontSize: 28, fontWeight: 'bold', marginBottom: 4 },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12 },
  tarjeta: { borderRadius: 14, padding: 16, width: '47%', borderWidth: 1, gap: 4 },
  tarjetaEmoji: { fontSize: 24, marginBottom: 4 },
  tarjetaValor: { color: '#fff', fontSize: 32, fontWeight: 'bold' },
  tarjetaLabel: { color: '#9090b0', fontSize: 13 },
  card: { backgroundColor: '#16162a', borderRadius: 14, padding: 16, borderWidth: 1, borderColor: '#252540', gap: 10 },
  cardTitulo: { color: '#fff', fontSize: 16, fontWeight: '600' },
  cardSubtitulo: { color: '#9090b0', fontSize: 12, marginTop: -6 },
  barraFila: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  barraLabel: { color: '#ccc', fontSize: 13, width: 90 },
  barraTrack: { flex: 1, height: 8, backgroundColor: '#252540', borderRadius: 4, overflow: 'hidden' },
  barraRelleno: { height: '100%', borderRadius: 4 },
  barraCantidad: { fontSize: 13, fontWeight: '600', width: 18, textAlign: 'right' },
  cardItemTop: { backgroundColor: '#16162a', borderRadius: 14, padding: 16, borderWidth: 1, borderColor: '#f59e0b40', gap: 4 },
  itemTopEtiqueta: { color: '#f59e0b', fontSize: 11, fontWeight: '700', letterSpacing: 0.8, marginBottom: 4 },
  itemTopTexto: { color: '#fff', fontSize: 18, fontWeight: '600' },
  itemTopSub: { color: '#9090b0', fontSize: 13 },
  vacio: { color: '#55556a', fontSize: 14 },
  botonCerrarSesion: { borderColor: '#ff4444', marginTop: 8 },
});